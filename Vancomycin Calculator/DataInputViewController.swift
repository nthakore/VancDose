...
class DataInputViewController: UIViewController {
    ...
    @IBOutlet weak var sCrField: SkyFloatingLabelTextField! {
        didSet {
            sCrField.placeholder = "Serum creatinine in mg/dL"
            sCrField.title = "Patient's serum creatinine"
        }
    }
    ...
    @IBAction func calculateButtonTouched(_ sender: Any) {
        if let patientInitialsText = patientInitials.text,
            ...
            let sCrText = sCrField.text,
            let weightDouble = Double(weightText),
            let heightDouble = Double(heightText),
            let ageInt = Int(ageText),
            let sCrDouble = Double(sCrText),
            maleButton.isSelected || femaleButton.isSelected {
            calculateButton.startLoadingAnimation()
            
            let newPatient = Patient(patientInitials: patientInitialsText, weight: weightDouble, height: heightDouble, age: ageInt, sCr: sCrDouble, isMaleGender: maleButton.isSelected)
